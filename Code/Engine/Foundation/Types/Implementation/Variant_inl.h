...
template<typename T, typename std::enable_if_t<ezVariantTypeDeduction<T>::classification == ezVariantClass::ObjectCast, int>>
...
EZ_ALWAYS_INLINE ezVariant::ezVariant(const void* value, const ezRTTI* pType)
{
  InitTypedPointer(const_cast<void*>(value), pType);
}
...
EZ_FORCE_INLINE void ezVariant::InitTypedObject(const T& value, ezTraitInt<0>)
{
  ...
  EZ_CHECK_AT_COMPILETIME_MSG((sizeof(StorageType) > sizeof(InlinedStruct::DataSize)) || TypeDeduction<T>::forceSharing, "value of this type should be stored inplace");
...
{
  typedef typename TypeDeduction<T>::StorageType StorageType;
  EZ_CHECK_AT_COMPILETIME_MSG((sizeof(StorageType) <= InlinedStruct::DataSize) && !TypeDeduction<T>::forceSharing, "value of this type should be stored inplace");
  EZ_CHECK_AT_COMPILETIME_MSG(TypeDeduction<T>::value == Type::TypedObject, "value of this type cannot be stored in a Variant");
  EZ_CHECK_AT_COMPILETIME_MSG(ezIsPodType<T>::value, "in place data needs to be POD");
  ezMemoryUtils::CopyConstruct(reinterpret_cast<T*>(&m_Data), value, 1);
