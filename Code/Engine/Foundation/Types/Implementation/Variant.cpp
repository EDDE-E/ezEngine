...
EZ_FORCE_INLINE ezUInt64 ComputeHashFunc::operator()<ezTypedObject>(const ezVariant& v, const void* pData, ezUInt64 uiSeed)
{
  ...
  EZ_ASSERT_DEV(pTypeInfo, "The type '{0}' was declared but not defined, add EZ_DEFINE_VARIANT_TYPE({0}); to a cpp to enable comparing of this variant type.", pType->GetTypeName());
  //TODO: 32 -> 64 hash upcast
  return pTypeInfo->Hash(pData);
}
...
EZ_FORCE_INLINE void CompareFunc::operator()<ezTypedObject>()
{
  ...
  if (A.m_pType == B.m_pType)
  {
    ...
    EZ_ASSERT_DEV(pTypeInfo, "The type '{0}' was declared but not defined, add EZ_DEFINE_VARIANT_TYPE({0}); to a cpp to enable comparing of this variant type.", A.m_pType->GetTypeName());
