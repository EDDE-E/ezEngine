...
#include "Reflection/Implementation/AbstractProperty.h"
...
ezResult ezOpenDdlUtils::ConvertToVariant(const ezOpenDdlReaderElement* pElement, ezVariant& out_result)
{
  ...
  if (pElement->IsCustomType())
  {
    ...
    if (const ezRTTI* pRTTI = ezRTTI::FindTypeByName(pElement->GetCustomType()))
    {
      ...
      {
        ezLog::Error("The type '{0}' was declared but not defined, add EZ_DEFINE_VARIANT_TYPE({0}); to a cpp to enable serialization of this variant type.", pElement->GetCustomType());
      }
    }
    else
    {
...
void ezOpenDdlUtils::StoreVariant(ezOpenDdlWriter& writer, const ezVariant& value, const char* szName /*= nullptr*/, bool bGlobalName /*= false*/)
{
  switch (value.GetType())
  {
    ...
    case ezVariant::Type::TypedObject:
    {
      ...
      {
        ezLog::Error("The type '{0}' was declared but not defined, add EZ_DEFINE_VARIANT_TYPE({0}); to a cpp to enable serialization of this variant type.", obj.m_pType->GetTypeName());
      }
    }
    return;
    default:
