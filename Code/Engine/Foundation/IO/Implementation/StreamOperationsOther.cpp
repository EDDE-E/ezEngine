...
inline void WriteValueFunc::operator()<ezTypedObject>()
{
  ...
  {
    EZ_REPORT_FAILURE("The type '{0}' was declared but not defined, add EZ_DEFINE_VARIANT_TYPE({0}); to a cpp to enable serialization of this variant type.", obj.m_pType->GetTypeName());
  }
}
...
inline void ReadValueFunc::operator()<ezTypedObject>()
{
  ...
  EZ_ASSERT_DEV(pTypeInfo, "The type '{0}' was declared but not defined, add EZ_DEFINE_VARIANT_TYPE({0}); to a cpp to enable serialization of this variant type.", sType);
